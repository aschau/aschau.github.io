...
## Product Filter (All Products / API Only)

Frontend toggle below the gauge. Stored in localStorage (`misery_product_filter`). In API mode:
- Status card shows only the Claude API component
- Status score recalculated from API component status only (degraded=+2, partial_outage=+4, major_outage=+6)
- System-level critical/major indicators still contribute (they affect the API too)
- Social signals remain unfiltered (posts rarely distinguish between API and chat product)
- `filterComponents()` and `isApiComponent()` in app.js handle the filtering
- Gauge window text updates to show "Claude API" prefix

## RSS Feed

Generated by the GitHub Action in `feed.xml` alongside `current.json` on the `misery-data` branch.
- URL: `https://raw.githubusercontent.com/aschau/aschau.github.io/misery-data/feed.xml`
- RSS 2.0 with Atom self-link
- Items: current misery score + up to 5 recent incidents
- TTL: 15 minutes (matches polling interval)
- `generateRssFeed()` in `.github/scripts/fetch-misery-data.js`
- Autodiscovery `<link rel="alternate">` in index.html `<head>`
- RSS button in header (SVG icon)
...
- `data/feed.xml` — generated RSS feed; live on `misery-data` branch
