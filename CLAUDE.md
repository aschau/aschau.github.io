...
- **Files**: `index.html`, `style.css`, `game.js` (core engine), `puzzles.js` (inline puzzle data + daily selection), `puzzles.json` (same data for validator), `share.js` (clipboard sharing), `generate.py` (puzzle generator), `validate.py` (puzzle validator), `debug.html` (local puzzle viewer — gitignored)
...
- **Fixed pieces**: Pre-placed mirrors (gold-colored with lock icon) that can't be moved. The beam interacts with them like normal mirrors. Stored in `puzzle.fixed` array as `[{r, c, type}]`. Rendered with `.fixed` CSS class. Click/rotate/remove is blocked on fixed cells. Not counted toward score. Explained in How to Play modal.
...
- **Scoring**: Par system with laser-themed labels — `par = minimum_solution + 1`. Labels shifted so all are reachable: Brilliant (min pieces, -1), Focused (par, 0), Aligned (+1), Scattered (+2), Refracted (+3+). Best score across resets is saved.
...
- **Share**: Narrative share text (spoiler-free). Shows score label (Brilliant/Focused/Aligned/Scattered/Refracted), piece count with score/par ratio, gem status, puzzle preview (fixed/targets/walls counts), and streak. Includes display name if set. Web Share API for native share sheet, clipboard fallback for copy. Share preview shown in win modal.
...
- **Puzzle generation**: `generate.py` (Python 3) uses a walls-first approach:
  ...
  After building the path, 1-2 mirrors are selected as fixed pieces (pre-placed on the board). The solver then finds the minimum additional pieces the player needs.
  Difficulty mix: 60% hard (4-6 mirrors, 2-4 walls, 1 fixed, min 4), 40% expert (5-7 mirrors, 1-3 walls, 1-2 fixed, min 4).
  Run: `python generate.py [count] [seed]`. Outputs both `puzzles.json` and `puzzles.js`.
...
- **Clear stats**: "Clear all stats & progress" button in How to Play modal. Uses inline confirm (button text changes to red "Are you sure?" for 3 seconds) instead of browser `confirm()` dialog. Clears all localStorage keys (`beamlab_data`, `beamlab_username`, `beamlab_seen_controls`).
...
- **Files**: `index.html`, `style.css`, `game.js` (core engine), `puzzles.js` (inline puzzle data + daily selection), `share.js` (clipboard sharing), `generate.py` (puzzle generator), `verify_puzzles.py` (puzzle validator), `debug.html` (local puzzle viewer — gitignored)
- **Puzzle data**: 365 themed puzzles stored inline in `puzzles.js` (one per day of the year). Each puzzle has a 2D `lines` array of tokens, a `goal` (prompt text), expected `output`, `par`, `difficulty`, and `id`. Do not edit `puzzles.js` manually. Distribution: ~315 medium, ~50 hard (no easy tier). Concepts: for loops, for+if, while loops, while+if, if/else branching, chain arithmetic. Puzzles are shuffled with fixed seed for even difficulty distribution across the year.
...
- **Interpreter**: `PseudoInterpreter` class — left-to-right evaluation (no operator precedence), integer division with floor, max 200 steps / 50 loop iterations (throws on infinite loops). Supports `let`, `return`, `while`, `if/else`, `for i = start to end { }` (inclusive range). Tracks `vars` (variable state) and `output` (return value). `resolveValue` throws on undeclared variables.
...
- **Daily puzzle**: Selected by date offset from `LAUNCH_EPOCH` in `puzzles.js`. Cycles through 365 puzzles (one per day of the year).
- **Scoring**: Par system with code-themed labels — `par = min_swaps + 3` computed from actual scramble. Labels: Genius (-3, perfect solve), Hacker (-2), Optimized (-1), Compiled (par), Verbose (+1), Spaghetti (+2+). Best score across resets is saved.
