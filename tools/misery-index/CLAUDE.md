...
- GUIDs are `{type}-{level}` (no date) so readers only see new items on level changes
- Score item pubDate is set to when the level last changed, not the current time
...
- `misery.module.js` — testable pure logic from app.js + fetch-misery-data.js (keep in sync with both)
