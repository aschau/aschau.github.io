...
- Gem: optional collectible costing 1-2 extra pieces. Solvable with and without. One-shot decision — board locks after first solve (no replays).
...
## Persistence
`localStorage` key `beamlab_data`: board state, bestScore, gemEverCollected, stats, streaks. Board locks after first solve — reset/undo disabled, no replays.
