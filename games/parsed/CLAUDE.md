...
- Interpreter: left-to-right evaluation (no operator precedence), integer division with floor. Supports `let`, `return`, `while`, `if/else`, `for i = start to end { }` (inclusive range). `resolveValue` throws on undeclared variables.
