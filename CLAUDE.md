...
- **Share**: Narrative share text (spoiler-free). Shows score label (Ace/Eagle/Birdie/Par/Bogey), piece count, gem status, puzzle preview (fixed/targets/walls counts), and streak. Includes display name if set. Web Share API for native share sheet, clipboard fallback for copy. Share preview shown in win modal.
...
- **Puzzle data**: 365 themed puzzles stored inline in `puzzles.js` (one per day of the year). Each puzzle has a 2D `lines` array of tokens, a `goal` (prompt text), expected `output`, `par`, `difficulty`, and `id`. Do not edit `puzzles.js` manually. Distribution: ~173 easy (arithmetic, chains), ~142 medium (while loops, accumulators), ~50 hard (while+if, complex). Concepts: arithmetic, if/else branching, while loops, while+if combos. Puzzles are shuffled with fixed seed for even difficulty distribution across the year.
