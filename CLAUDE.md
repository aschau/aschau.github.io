...
- **Testing**: `npm test` runs Jest (282 tests). Each game/tool extracts pure logic into a `.module.js` file (UMD-compatible, used by both tests and browser). Tests live in `__tests__/`. CI runs on push/PR via `.github/workflows/tests.yml`.
...
- **About** (`sections/about.html`): **MTG board layout** with grid zones — Commander (left, your personal card with P/T box), Battlefield (center, 6 company `.gc` cards representing career history), Emblem (right, social-links card), Skills (bottom row of skill "land" cards). On mobile: vertical scroll, 2-col battlefield grid, 3-col skills grid.
- **Work** (`sections/work.html`): **Deck-box hand** at the bottom (5 3D deck boxes for Blizzard, MobilityWare, Sega Atlus, Trigger Global, Super Toy Box) with a dynamic deck title above. Active deck "opens" to show its cards in the carousel above.
- **Personal** (`sections/personal.html`): Same deck-box pattern, 5 decks: Friend Castle, Waxheart, Games & Tools, Website, College.
...
- **Layer 0 (WORLD)**: cabinets visible, world character walks to cabinets. LEFT/RIGHT switches cabinet.
- **Layer 1**: context-dependent — **DECKS** on Work/Personal (hand-card boxes), **CARDS** elsewhere (card carousel). UP enters this layer from world. On Work/Personal, the pixel sprite sits on top of the active deck.
...
- `highlightCard()`, `highlightTab()` — active-state + sprite positioning
- `positionSpriteAtCard(card, above)` — unified sprite positioning (used for board layout cards and deck boxes)
...
- **Content sprite** (`.content-sprite`, 48px): appears in content area, moves between interactive elements (cards, deck boxes). Has a SPACE/TAP text hint; hidden on mobile and when on decks layer (flipping N/A).
